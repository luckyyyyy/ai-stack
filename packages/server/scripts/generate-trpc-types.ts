...
const typesOutFile = path.resolve(
  rootDir,
  "../types/src/trpc/@generated/app-router.ts",
);
...
const generateTypes = () => {
  ...
  const typesImportMap = new Map<string, Set<string>>();
  for (const [specifier, names] of importMap.entries()) {
    const targetSpecifier =
      specifier === "@acme/types"
        ? "../../index"
        : specifier === "zod"
          ? "zod"
          : null;
    ...
    const set = typesImportMap.get(targetSpecifier) ?? new Set<string>();
    for (const name of names) {
      set.add(name);
    }
    typesImportMap.set(targetSpecifier, set);
  }

  const typesImports = Array.from(typesImportMap.entries())
    .map(([specifier, names]) => {
      const named = Array.from(names).sort().join(", ");
      return `import { ${named} } from "${specifier}";`;
    })
...
`;
  ...
  fs.writeFileSync(typesOutFile, typesContent);
};
