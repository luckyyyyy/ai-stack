...
export const createAppRouter = (resolve?: Resolver): AnyRouter => {
  const logger = new Logger("RouterLoader");
  ...
  const routerEntries = getRouterRegistry().map((routerClass) => {
    ...
    const instance: RouterInstance = resolve
      ? resolve(RouterCtor)
      : new RouterCtor();
    const proto = Object.getPrototypeOf(instance);

    const procedures: Record<string, AnyProcedure> = {};
    const procedureKinds: Record<string, string> = {};
    for (const methodName of Object.getOwnPropertyNames(proto)) {
      ...
      if (meta.kind === "query") {
        ...
        procedureKinds[methodName] = "query";
      }
      ...
      if (meta.kind === "mutation") {
        ...
        procedureKinds[methodName] = "mutation";
      }
    }

    const procedureNames = Object.keys(procedures);
    logger.log(
      `${routerClass.name} {${alias}} — ${procedureNames.map((n) => `${n}(${procedureKinds[n]})`).join(", ")}`,
    );
